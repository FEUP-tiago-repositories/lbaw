    ...
    <div class="p-4">
        ...
        @elseif($booking->isFuture())
            ...
            <button type="button" data-booking-id="{{ $booking->id }}" data-space-name="{{ $booking->space->title }}"
                ...
                data-time="{{ $booking->schedule->start_time->format('H:i') }}" data-duration="{{$booking->total_duration}}"
                data-amount="{{ $booking->payment->value }}" data-space-id="{{ $booking->space_id }}"
                data-schedule-id="{{ $booking->schedule_id }}" onclick="openCancelModalFromData(this)"
        ...
        @elseif($booking->isPast())
            ...
            <a  
                href="{{ route('spaces.show', $booking->space_id) }}"
