...
<div class="bg-white rounded-lg shadow-md overflow-hidden flex-none w-90">
    ...
    <div class="h-48 overflow-hidden bg-gray-200">
        @if($booking->space && $booking->space->media->isNotEmpty())
            <img src="{{ $booking->space->media->first()->media_url }}" alt="{{ $booking->space->title }}"
    ...
    <div class="p-4">
        ...
        @elseif($booking->isFuture())
            ...
            <div class="flex gap-2 mt-4">
                ...
                <button type="button" data-booking-id="{{ $booking->id }}" data-space-name="{{ $booking->space->title }}"
                    data-date="{{ $booking->schedule->start_time->format('d/m/Y') }}"
                    data-time="{{ $booking->schedule->start_time->format('H:i') }}"
                    data-duration="{{$booking->total_duration}}" data-amount="{{ $booking->payment->value }}"
                    data-space-id="{{ $booking->space_id }}" data-schedule-id="{{ $booking->schedule_id }}"
        ...
        @elseif($booking->isPast())
            ...
            <div class="flex gap-2 mt-4">
                <a href="{{ route('spaces.show', $booking->space->id) }}" class="flex flex-1 border bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-200 hover:text-black
                        transition text-sm font-medium cursor-pointer justify-center">
                    Write a review
                </a>
                <button onclick="alert('Repeat booking feature - Coming soon!')"
                    class="flex-1 bg-emerald-800 text-white px-4 py-2 rounded-lg hover:bg-emerald-800 hover:text-black transition text-sm font-medium cursor-pointer">
                    Repeat reservation
                </button>
            </div>
        @endif
